...
const MainMenu = styled(Box)<{ show: boolean }>(({ theme, show }) => ({
  ...
  "& .simplebar-scrollbar:before": { background: theme.palette.text.primary },
}));

const SecondarySideBar = styled(Drawer)(({ theme }) => ({
  zIndex: theme.zIndex.drawer + 1,
  "& .MuiPaper-root": { left: 80, minWidth: 200 },
}));
...
const SubMenuItem = styled(FlexBox)<{ active?: boolean }>(({ theme, active }) => ({
  cursor: "pointer",
  alignItems: "center",
  padding: "0.6rem 1rem",
  "& div": {
...
const DashboardSideBar: FC<SideNavBarProps> = ({
  ...
  if (downMd) {
    const handleClose = () => {
      return openSecondarySideBar ? setOpenSecondarySideBar(false) : closeMobileSideBar();
    };
  ...
  return (
    <Fragment>
      ...
      <SecondarySideBar open={openSecondarySideBar} onClose={() => setOpenSecondarySideBar(false)}>
        {secondarySideBarContent}
      </SecondarySideBar>
    </Fragment>
  );
