...
interface ProjectCard2Props {
  handleMoreClick: (event: MouseEvent<HTMLButtonElement>) => void;
}

const ProjectCard2: FC<ProjectCard2Props> = ({ handleMoreClick }) => {
  return (
    <Card sx={{ padding: 2, boxShadow: 2 }}>
      <FlexBox alignItems="center" justifyContent="space-between">
        <Small>July 2, 2020</Small>
        <IconButton sx={{ padding: 0 }} onClick={handleMoreClick}>
          <MoreHoriz />
        </IconButton>
      </FlexBox>
